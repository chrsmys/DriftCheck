...
public enum RetentionMode: Equatable {
    /**
     Checks that anchor and tethered objects are nil when the anchor leaves the hierarchy.
     if the anchor is never added to the hierarchy then the behavior will defer to onDealloc.
    ...
    /**
     ...
     once it leaves the hierarchy. In this case the tethered objects will be checked for drift when the anchor is deallocated.
     */
    case onDealloc
    /**
